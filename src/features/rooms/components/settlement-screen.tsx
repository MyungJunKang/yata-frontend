...
// 백엔드 /rooms/{id}/settlement 가 허용하는 이미지 타입·크기 — multipart 로 그대로 전송.
...
export function SettlementScreen({ roomId }: Props) {
  ...
  // 선택된 영수증 — 정산 게시 시 multipart 바이너리로 함께 전송.
  const [imageFile, setImageFile] = useState<File | null>(null);
  // 파일 선택 시점 타입/크기 검증 실패 메시지.
    ...
    createMutation.mutate(
      {
        ...
        // 영수증 이미지는 별도 업로드 없이 정산 multipart 에 바이너리로 그대로 첨부.
        ...
        <section className="flex flex-col gap-3 rounded-2xl bg-bg-normal p-[18px] shadow-sm">
          ...
          {pickError && (
            ...
            >
              ...
              <span className="flex-1">{pickError}</span>
            </div>
          )}
        </section>
